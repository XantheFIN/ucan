...
bool CanAsyncWrapper::getSendAcknMessage(SharedCanMessage& aMsg, uint16_t aTransactionId, uint32_t aTimeoutMs){
    if(!pimpl->mPortIsOpen){
    	return false;
    }
	return pimpl->mCan->getSendAcknMessage(aMsg, aTransactionId, aTimeoutMs);
}
